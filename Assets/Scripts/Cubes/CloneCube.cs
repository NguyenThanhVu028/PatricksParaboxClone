...
public class CloneCube : ContainerCube
{
    ...
    protected override void DrawChildCubes(Rect position, int priority, float depth, float exposure, Rect? scissorRect, int minSize)
    {
        ...
        mainContainerCube.DrawInnerCubes(position, priority, depth, exposure, CustomTextureRenderer2D.GetOverlapRect(scissorRect, position), minChildCubePixelToRender);

        DrawEnterCube(position, priority + normalEnterPriority, depth + normalEnterDepthOffset, exposure, scissorRect, minSize);
        ...
        //{
        //    emptyCube.Draw(Relativity.CRectFromPRect(position, emptyCube.RelativeScale, emptyCube.RelativePosition), priority, depth, exposure, scissorRect);
        //}
        ...
        //        continue;
        //    }
        //    childCube.Cube.Draw(Relativity.CRectFromPRect(position, childCube.Cube.RelativeScale, childCube.Cube.RelativePosition), priority, depth, exposure, scissorRect);
        //}
        ...
        //        movingCube.Draw(Relativity.CRectFromPRect(position, idealRScl, idealRPos), priority, depth + movingChildCubesDepthOffset, exposure, CustomTextureRenderer2D.GetOverlapRect(scissorRect, position));
        //    }
        //    else movingCube.Draw(Relativity.CRectFromPRect(position, movingCube.RelativeScale, movingCube.RelativePosition), priority, depth + movingChildCubesDepthOffset, exposure, scissorRect);
        //}

        //// Draw the requested cube
        //if (requestedCube != null)
        //{
        //    requestedCube.Draw(Relativity.CRectFromPRect(position, requestedCube.RelativeScale, requestedCube.RelativePosition), priority, depth - 0.1f, exposure, scissorRect);
        //}
    }
    ...
    public override void DrawExitCube(Rect position, int priority, float depth, float exposure, Rect? scissorRect, float minSize = -1)
    {
        if (mainContainerCube != null) mainContainerCube.DrawExitCube(position, priority, depth, exposure, scissorRect, minSize);
    }

    public override void DrawAlterEnterCube(Rect position, int priority, float depth, float exposure, Rect? scissorRect, float minSize = -1)
    {
        if (mainContainerCube != null) mainContainerCube.DrawAlterEnterCube(position, priority, depth, exposure, scissorRect, minSize);
    }
    ...
    public override float RequestToMove(Vector2 cRPos, Vector2 cRScl, Cube requestedCube, CubeMovement.GridDirections direction, bool external = false, bool specialMove = false)
    {
        ...
        if (isHorizFlipped != mainContainerCube.IsHorizFlipped)
        {
            ...
            requestedCube.PreviousParents.Add(new(CubeMovement.LayerDirections.In, this, new(-mainContainerCube.RelativePosition.x, -mainContainerCube.RelativePosition.y), Vector2.one, true));
        }
        else requestedCube.PreviousParents.Add(new(CubeMovement.LayerDirections.In, this, new(-mainContainerCube.RelativePosition.x, -mainContainerCube.RelativePosition.y), Vector2.one, false));
        ...
        //enterCube = requestedCube;
        //if (requestedCube.Parent == mainContainerCube)
        //{
        //    enterCube = requestedCube;
        //    if (mainContainerCube.EnterCube == requestedCube) mainContainerCube.EnterCube = null;
        //}
        //if (requestedCube.Parent.EnterCube == requestedCube) requestedCube.Parent.EnterCube = null;
